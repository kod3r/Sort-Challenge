...
import json
import unittest
from nodes import PairedNode
from nodes import String
from nodes import Listing
...
class TestItem(unittest.TestCase):
    ...
    def test_process_string(self):
        ...
        self.assertEqual(item.process_string(test_string), 'helloaaa333aaa333.')
        del item

    def test_refine(self):
        item = Item()
        test_string = 'jOsePH!!!@@@###$$$%%%^^^&&&***((()))dUNg'
        self.assertEqual(item.refine(test_string), 'josephdung')
        del item
...
class TestBinarySearchTree(unittest.TestCase):
    def setUp(self):
        self.numeric_tree = PairedNode()
        self.product_tree = PairedNode()
    ...
    def test_traverseNodes(self):
        node_counter = [] 
        self.numeric_tree.traverseNodes(lambda x: node_counter.append(1))
